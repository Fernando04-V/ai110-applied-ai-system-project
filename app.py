...
app.py
------
Glitchy Guesser — extended with a rule-based AI coaching system,
confidence scoring, input guardrails, and game logging.
...
st.caption("A number guessing game with an AI reliability coach.")
...
range_map = {"Easy": (1, 20), "Normal": (1, 100), "Hard": (1, 50)}
low, high = range_map[difficulty]

st.sidebar.caption(f"Range: {low} to {high}")
st.sidebar.caption(f"Attempts allowed: {attempt_limit}")

# ── Session state init ─────────────────────────────────────────────────────────
# FIX: Check if difficulty changed and reset the game if so.
# This fixes the bug where the secret number stayed the same when difficulty changed.
...
st.caption("The coach analyzes your guess pattern and suggests what to do next.")
ask_ai = st.button("Get AI Strategy Tip")

if ask_ai:
    int_history = [e for e in st.session_state.history if isinstance(e, tuple)]
...
if st.session_state.confidence is not None:
    ...
    if conf >= 0.7:
        ...
        label = "Random"
    st.metric("Strategy Score", f"{points:.2f} pts", label)

# ── New Game ───────────────────────────────────────────────────────────────────
# FIX: Refactored with AI assistance to allow resetting the game at any time.
if new_game:
    st.session_state.secret = random.randint(low, high)
    st.session_state.attempts = 0
    st.session_state.score = 0
...
# ── Submit guess ───────────────────────────────────────────────────────────────
if submit:
    # Guardrail: validate input before processing
...
st.divider()
st.caption("Original buggy game generated by AI. Fixed and extended as a reliability system.")